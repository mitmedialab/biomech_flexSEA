...
void tx_cmd_biomech_r(uint8_t *shBuf, uint8_t *cmd, uint8_t *cmdType, \
						uint16_t *len, uint8_t subcmd, float desiredJointAngleDeg_f, float desiredJointK_f, float desiredJointB_f) {
	...
	SPLIT_16((uint16_t) (desiredJointB_f*INT_SCALING), shBuf, &index);
	(*len) = index;
}
...
void tx_cmd_biomech_w(uint8_t *shBuf, uint8_t *cmd, uint8_t *cmdType, \
						uint16_t *len, uint32_t *flags, uint8_t lenFlags, uint8_t subcmd,  float desiredJointAngleDeg_f, float desiredJointK_f, float desiredJointB_f)
{
...
//	uint16_t rawTau = REBUILD_UINT16(msgBuf, &index);
	...
	act1.desiredJointB = raw_desiredJointB;
...
//	act1.tauDes = ((float) rawTau)/INT_SCALING;
	...
	act1.motorOnFlag = 1; //turn motor flag on or off.
	act1.commandTimer = 0;
